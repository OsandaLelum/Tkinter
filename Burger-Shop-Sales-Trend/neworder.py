...
class BurgerOrderingSystem:
    def __init__(self, master):
        ...
        self.cart = []
        BurgerOrderingSystem.initialize_database()
        self.create_widgets()
...
# Then, within your BurgerOrderingSystem class or wherever appropriate:
    ...
    def checkout(self):
     if not self.cart:
        ...
        for item in self.cart:
            ...
            item_name = item
            quantity = 1  # Assuming each item is only added once; adjust if necessary
            price = details['price']
            total_price += price
            
            c.execute('''
                INSERT INTO sales (item_name, quantity, total_price, sale_date) VALUES (?, ?, ?, ?)
            ''', (item_name, quantity, price, current_time))
        
        conn.commit()
        conn.close()
        messagebox.showinfo("Checkout Successful", f"Your order has been saved. Total price: LKR {total_price}")
        
        # Optionally clear the cart after checkout
        self.cart.clear()
        self.cart_label.configure(text="Cart: 0 items")
        subprocess.run(["python", "payment.py"], check=True)
        # Here you can call subprocess to open the payment processing script if necessary
        # subprocess.run(["python", "payment.py"], check=True)
